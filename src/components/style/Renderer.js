...
let defaultDrawIcon = function ({qrcode, params, title, icon}) {
    ...
    if (icon) {
        ...
        if (icon && iconEnabled) {
            ...
            pointList = <>
                <path key={id++} d={sq25} stroke="#FFF" strokeWidth={100 / iconSize * 1} fill="#FFF" transform={`translate(${String(iconXY)}, ${String(iconXY)}) scale(${String(iconSize / 100)}, ${String(iconSize / 100)})`} />
                <g key={id++}>
                    <defs>
...
let builtinDrawIcon = function ({qrcode, params, title, icon}) {
    ...
    if (icon) {
        ...
        if (icon && iconMode) {
            const randomIdDefs = getIdNum();
            const randomIdClips = getIdNum();
            pointList.push(<path key={id++} d={sq25} stroke="#FFF" strokeWidth={100 / iconSize * 1} fill="#FFF" transform={`translate(${String(iconXY)}, ${String(iconXY)}) scale(${String(iconSize / 100)}, ${String(iconSize / 100)})`} />);
            pointList.push(
                <g key={id++}>
                    <defs>
                        <path id={"defs-path" + randomIdDefs} d={sq25} fill="#FFF" transform={`translate(${String(iconXY)}, ${String(iconXY)}) scale(${String(iconSize / 100)}, ${String(iconSize / 100)})`} />                    </defs>
